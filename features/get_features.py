...
class Features():
    ...
    class Measures():
        ...
        def get_physio_data(self):
            ...
            for measure in PHYSIO_MEASURES:
                ...
                # print(query_database('''SELECT tablename# FROM pg_catalog.pg_tables# WHERE schemaname = 'mimiciii_demo''''))
                physio_data = query_database(
                    f"""SELECT 
                                                hadm_id,
                                                avg(valuenum) AS AVG_{measure['name']},
                                                stddev(valuenum) AS STD_{measure['name']}
                                                FROM mimiciii_demo.chartevents
                                                WHERE itemid in ({measure['itemid']})
                                                GROUP BY hadm_id
        ...
        def get_measures(self):
            ...
            print(f"Lab data {lab_data.shape}")
            print(f"Lab data {lab_data.shape}")
    ...
    class ICUData():
        ...
        def get_icu_data(self):
            ...
            for df in icu_dfs:
                if 'hadm_id' in df.index.names:
                    df.reset_index(inplace=True, drop=True)  # Resets hadm_id to be a column if it's part of the index.
                    # Ensure all hadm_id columns are of the same type
